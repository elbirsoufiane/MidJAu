...
def process_batch(batch, start_index):
    ...
    time.sleep(30)
    log("\n👁 Triggering U4 upscaled images is in progress...")
    ...
    for _ in range(len(queue)):
        ...
        for msg in reversed(messages):
            if msg.get("author", {}).get("id") != MIDJOURNEY_APP_ID:
                continue
            for q in queue:
                if q["u4_clicked"] and not q["image_saved"] and msg.get("message_reference", {}).get("message_id") == q["message_id"]:
...
def main(user_email: str, prompts_file: str):
    ...
    log("🟢 MidjourneyU4 mode started running ...")
    check_cancel()
