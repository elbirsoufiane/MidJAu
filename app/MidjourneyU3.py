...
# print("🟢 MidjourneyU3 mode started running ...", flush=True)
...
#     if res.status_code == 200:
#         filepath = os.path.join(OUTPUT_DIR, f"{index}_U3{ext}")
...
#     time.sleep(30)
#     print("\n👁 Triggering U3 upscaled images is in progress...", flush=True)
...
#                             break
#         if all(q["u3_clicked"] for q in queue):
#             break
#         time.sleep(5)
...
#                 continue
#             for q in queue:
#                 if q["u3_clicked"] and not q["image_saved"]:
...
def process_batch(batch, start_index):
    ...
    time.sleep(30)
    log("\n👁 Triggering U3 upscaled images is in progress...")
    for _ in range(len(queue)):
        check_cancel()
        messages = get_messages()
        for msg in reversed(messages):
    ...
    for _ in range(len(queue)):
        ...
        for msg in reversed(messages):
            if msg.get("author", {}).get("id") != MIDJOURNEY_APP_ID:
                continue
            for q in queue:
                if q["u3_clicked"] and not q["image_saved"] and msg.get("message_reference", {}).get("message_id") == q["message_id"]:
                    attachments = msg.get("attachments", [])
                    if attachments:
                        url = attachments[0]["url"]
