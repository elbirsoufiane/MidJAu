...
# print("🟢 MidjourneyU1 mode started running ...", flush=True)
...
#     if res.status_code == 200:
#         filepath = os.path.join(OUTPUT_DIR, f"{index}_U1{ext}")
...
#                             break
#         if all(q["u1_clicked"] for q in queue):
#             break
#         time.sleep(5)
...
#                 continue
#             for q in queue:
#                 if q["u1_clicked"] and not q["image_saved"]:
    ...
    for _ in range(len(queue)):
        ...
        for msg in reversed(messages):
            if msg.get("author", {}).get("id") != MIDJOURNEY_APP_ID:
                continue
            for q in queue:
                if q["u1_clicked"] and not q["image_saved"] and msg.get("message_reference", {}).get("message_id") == q["message_id"]:
                    attachments = msg.get("attachments", [])
                    if attachments:
                        url = attachments[0]["url"]
...
def main(user_email: str, prompts_file: str):
    ...
    log("🟢 MidjourneyU1 mode started running ...")
    check_cancel()
