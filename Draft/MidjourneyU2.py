...
print("🟢 MidjourneyU2 script started...", flush=True)
...
def process_batch(batch, start_index):
    ...
    time.sleep(30)
    print("⏳ Waiting for U2 buttons...", flush=True)
    ...
    for _ in range(len(queue)):
        ...
        for msg in reversed(messages):
            if msg.get("author", {}).get("id") != MIDJOURNEY_APP_ID:
                continue
            for q in queue:
                if q["u2_clicked"] and not q["image_saved"]:
...
def main():
    ...
    for i in range(0, len(prompts), 10):
        ...
        print(f"\n🚀 Batch {i//10 + 1} - {len(batch)} prompts...", flush=True)
        try:
